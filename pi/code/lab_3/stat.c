...
void cdf(double v[], int trials) {
    for (int i = 0; i < TWO_DICE_SUM; i++) v[i] = 0.0;
    for (int i = 0; i < trials; i++) {
        int a = rand() % 6;
        a += rand() % 6;
        v[a] += 1.0 / trials;
    }
    for (int i = 1; i < TWO_DICE_SUM; i++) v[i] += v[i - 1];
}
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
    for(int i = 0; i < n; i++){
        printf("%2i |", i + x_start);
        for(int k = 0; k < round((v[i]/y_scale)); k++)
            printf("%c", mark);
...
void monty_hall(int n, int *p_switch_wins) {
    ...
    *p_switch_wins=0;
    for (int i=0; i<n; i++)
    {
        reward=rand()%3;
        chosen=rand()%3;
        if (chosen!=reward) (*p_switch_wins)++;
    }
}
