...
void* patient_thread_work(void *arg) {
    ...
    while (true) {
        ...
        if (in_waiting_room < 3) {
            in_waiting_room++;
            pos_in_queue = in_waiting_room;
...
void* doctor_thread_work(void *arg) {
    while (1) {
        ...
        while (!(num_patients_waiting == 3 && in_first_aid_kit >= FIRST_AID_KIT_EMPTY_INDICATOR) &&
        ...
        if (num_patients_waiting == 3 && in_first_aid_kit >= FIRST_AID_KIT_EMPTY_INDICATOR) {
            printf("[%0.10f] - Lekarz: konsultuję pacjentów %d, %d, %d\n",
                   get_time_elapsed(), patients_ready[0], patients_ready[1], patients_ready[2]);

            sleep(rand() % 3 + 2); // 2–4s
            in_first_aid_kit -= 3;
            if (in_first_aid_kit < FIRST_AID_KIT_EMPTY_INDICATOR) {
