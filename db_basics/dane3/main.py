...
def subjectsStudents():
    SUBJECTS_COUNT=1760
    user_start = 1
    rise = 20
    with open("SubjectStudents.csv", mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=["SubjectID", "UserID"])
        writer.writeheader()
        # 20 students per subject
        # change students assigned after 28 studies
        i = 0
        for subjectID in range(1, SUBJECTS_COUNT + 1):
            if i % 28 == 0:
                user_start += rise
            for user_id in range(user_start, user_start + rise):
                if user_id > 1000:
                    return
                writer.writerow({"SubjectID": subjectID, "UserID": user_id})
            i += 1
