...
static void initCDCL(struct solver *S, int n, int m) {
  ...
  /* Dynamic sizing: arrays(~9n) + clauses(m*7) + lemmas + margin.
     For large instances (100×100 grid, 60K vars) CDCL needs substantial
     room for learned lemmas, so we allocate generously. */
  S->mem_max = 10 * (n + 1) + m * 7 + 4000000;
  S->mem_used = 0;
  S->nLemmas = 0;
  S->nConflicts = 0;
  S->maxLemmas = 2000;
...
static int can_mirror[MAX_GRID][MAX_GRID]; /* 1 = candidate mirror position */

/* =========================================================================
   Part 3a: Beam reachability pruning (0-1 BFS)

   Compute which '.' cells can usefully be mirrors.  A mirror at (i,j) is
   useful only if a laser beam can reach it AND after reflection can reach
   a cat, with total mirrors on that path <= L.

   fwd_dist[i][j][d] = min mirrors to route a beam from any laser to
                        state "beam enters (i,j) going direction d"
   bwd_dist[i][j][d] = min mirrors to route a beam from state "beam
                        enters (i,j) going direction d" to any cat

   Both use 0-1 BFS (deque): straight-through = cost 0, bounce = cost 1.

   Cell (i,j) can be a mirror iff there exist d_in, mirror_type such that:
     fwd[i][j][d_in] + 1 + bwd[next_cell][d_out] <= L
   where d_out = reflection of d_in through mirror_type.
   ========================================================================= */

#define INF 999999
#define DEQUE_SZ (MAX_GRID * MAX_GRID * 4 * 2)

static int fwd_dist[MAX_GRID][MAX_GRID][4];
static int bwd_dist[MAX_GRID][MAX_GRID][4];

/* "/" reflection: incoming d → outgoing d */
static const int slash_refl[4] = {DE, DW, DN, DS}; /* DN→DE, DS→DW, DE→DN, DW→DS */
/* "\" reflection: incoming d → outgoing d */
static const int back_refl[4] = {DW, DE, DS, DN};  /* DN→DW, DS→DE, DE→DS, DW→DN */

/* Deque for 0-1 BFS: stores packed (i*W+j)*4+d */
static int deque[DEQUE_SZ];
static int dq_head, dq_tail, dq_sz;

static void dq_init(void) { dq_head = 0; dq_tail = 0; dq_sz = 0; }
static void dq_push_front(int v) {
  dq_head = (dq_head - 1 + DEQUE_SZ) % DEQUE_SZ;
  deque[dq_head] = v;
  dq_sz++;
}
static void dq_push_back(int v) {
  deque[dq_tail] = v;
  dq_tail = (dq_tail + 1) % DEQUE_SZ;
  dq_sz++;
}
static int dq_pop_front(void) {
  int v = deque[dq_head];
  dq_head = (dq_head + 1) % DEQUE_SZ;
  dq_sz--;
  return v;
}

static void compute_pruning(void) {
  int i, j, d, k;
  ...
  /* --- Forward BFS from lasers --- */
  dq_init();
  for (i = 0; i < gH; i++)
    for (j = 0; j < gW; j++) {
      int ld = -1;
      if      (grid[i][j] == '>') ld = DE;
      else if (grid[i][j] == '<') ld = DW;
      else if (grid[i][j] == 'A') ld = DN;
      else if (grid[i][j] == 'V') ld = DS;
      if (ld < 0) continue;
      int ni = i + dx[ld], nj = j + dy[ld];
      if (inside(ni, nj) && !is_blocking(ni, nj) && fwd_dist[ni][nj][ld] > 0) {
        fwd_dist[ni][nj][ld] = 0;
        dq_push_back((ni * gW + nj) * 4 + ld);
      }
    }
  ...
  while (dq_sz > 0) {
    ...
    j = cell % gW;
    i = cell / gW;
    int cost = fwd_dist[i][j][d];

    /* Option 1: no mirror at (i,j) — beam continues straight */
    {
      int ni = i + dx[d], nj = j + dy[d];
      if (inside(ni, nj) && !is_blocking(ni, nj)) {
        if (cost < fwd_dist[ni][nj][d]) {
          fwd_dist[ni][nj][d] = cost;
          dq_push_front((ni * gW + nj) * 4 + d);
        }
      }
    }
    /* Option 2: '/' mirror at (i,j) — only if cell is '.' */
    if (grid[i][j] == '.') {
      int d2 = slash_refl[d];
      int ni = i + dx[d2], nj = j + dy[d2];
      if (inside(ni, nj) && !is_blocking(ni, nj)) {
        if (cost + 1 < fwd_dist[ni][nj][d2]) {
          fwd_dist[ni][nj][d2] = cost + 1;
          dq_push_back((ni * gW + nj) * 4 + d2);
        }
      }
    }
    /* Option 3: '\' mirror at (i,j) */
    if (grid[i][j] == '.') {
      int d2 = back_refl[d];
      int ni = i + dx[d2], nj = j + dy[d2];
      if (inside(ni, nj) && !is_blocking(ni, nj)) {
  ...
  /* --- Backward BFS from cats ---
     We want bwd_dist[i][j][d] = min mirrors to get beam entering (i,j) in
     direction d to some cat.  BFS on reversed edges of the forward graph. */
  dq_init();
  for (i = 0; i < gH; i++)
    for (j = 0; j < gW; j++)
      if (grid[i][j] == 'O')
  ...
  while (dq_sz > 0) {
    ...
    j = cell % gW;
    i = cell / gW;
    int cost = bwd_dist[i][j][d];

    /* Reverse edges: who could have sent a beam to (i,j) going d?
       The beam came from predecessor (pi,pj) = (i - dx[d], j - dy[d]).
       At (pi,pj), the beam either passed through or bounced. */
    int pi = i - dx[d], pj = j - dy[d];
    if (!inside(pi, pj) || is_blocking(pi, pj))
      continue;

    /* Reverse of straight-through: beam was at (pi,pj) going d */
    if (cost < bwd_dist[pi][pj][d]) {
      bwd_dist[pi][pj][d] = cost;
      dq_push_front((pi * gW + pj) * 4 + d);
    }
    /* Reverse of '/' bounce at (pi,pj): beam arrived going d_in,
       '/' reflected to d.  slash_refl is its own inverse. */
    if (grid[pi][pj] == '.') {
      int d_in = slash_refl[d];
      if (cost + 1 < bwd_dist[pi][pj][d_in]) {
        bwd_dist[pi][pj][d_in] = cost + 1;
        dq_push_back((pi * gW + pj) * 4 + d_in);
      }
    }
    /* Reverse of '\' bounce at (pi,pj) */
    if (grid[pi][pj] == '.') {
      int d_in = back_refl[d];
      if (cost + 1 < bwd_dist[pi][pj][d_in]) {
        bwd_dist[pi][pj][d_in] = cost + 1;
        dq_push_back((pi * gW + pj) * 4 + d_in);
      }
  ...
  /* --- Determine which '.' cells can usefully be mirrors --- */
  int total_empty = 0, pruned = 0;
  for (i = 0; i < gH; i++)
    for (j = 0; j < gW; j++) {
      can_mirror[i][j] = 0;
      if (grid[i][j] != '.') continue;
      total_empty++;
      /* Check: exists d_in, mirror_type such that
         fwd[i][j][d_in] + 1 + bwd[next][d_out] <= L */
      for (k = 0; k < 4; k++) {
        int d_in = k;
        if (fwd_dist[i][j][d_in] >= INF) continue;
        /* '/' mirror */
        {
          int d_out = slash_refl[d_in];
          int ni = i + dx[d_out], nj = j + dy[d_out];
          if (inside(ni, nj) && !is_blocking(ni, nj))
            if (fwd_dist[i][j][d_in] + 1 + bwd_dist[ni][nj][d_out] <= gL) {
              can_mirror[i][j] = 1;
              break;
            }
        }
      ...
      if (!can_mirror[i][j]) pruned++;
    }

  fprintf(stderr, "c pruning: %d/%d empty cells pruned (%d candidates remain)\n",
          pruned, total_empty, total_empty - pruned);

  /* Diagnostic: count reachable R-states */
  int fwd_reach = 0, bwd_reach = 0, both_reach = 0;
  for (i = 0; i < gH; i++)
    for (j = 0; j < gW; j++)
      if (!is_blocking(i, j))
        for (d = 0; d < 4; d++) {
          int f = (fwd_dist[i][j][d] < INF);
          int b = (bwd_dist[i][j][d] < INF);
          fwd_reach += f;
          bwd_reach += b;
          both_reach += (f && b);
        }
  fprintf(stderr, "c R-states: fwd_reachable=%d bwd_reachable=%d both=%d total_non_wall=%d\n",
          fwd_reach, bwd_reach, both_reach, (gH * gW - pruned - total_empty) * 4);
}
...
static void build_sat(struct solver *S) {
  ...
  for (i = 0; i < gH; i++) {
    for (j = 0; j < gW; j++) {
      ...
      if (c == '#') {
        ...
        clause1(S, -var_M(i, j));
      } else if (c == '.' && !can_mirror[i][j]) {
        /* Pruned cell: cannot be a useful mirror position */
        clause1(S, -var_M(i, j));
      }
    }
  }
  ...
  /* --- Section 5: at-most-L mirrors (Sinz counter) --- */
  if (gL == 0) {
...
/* After verify fail (phantom cycle): find phantom mirrors (M=1 but not on
   any real light path) and add a no-good clause blocking that placement */
static void add_nogood_phantom(struct solver *S) {
...
int main(void) {
  ...
  /* Prune empty cells: only keep those reachable from laser AND cat */
  compute_pruning();

  /* Collect empty cells that survived pruning (candidate mirror positions) */
  n_mirror_cells = 0;
  for (i = 0; i < gH; i++)
    for (j = 0; j < gW; j++)
      if (grid[i][j] == '.' && can_mirror[i][j]) {
        mirror_cells[n_mirror_cells][0] = i;
        mirror_cells[n_mirror_cells][1] = j;
        n_mirror_cells++;
      }

  /* Always use Sinz counter for cardinality constraint */
  sc_base = 1 + 6 * gH * gW;
  int nVars = 6 * gH * gW + n_mirror_cells * gL;
  int nClauses = gH * gW * 40 + n_mirror_cells * gL * 4 + 1000;
  fprintf(stderr, "c Sinz counter: %d aux vars, %d total vars\n", n_mirror_cells * gL, nVars);
  ...
  for (int iter = 0; iter < max_iters; iter++) {
    ...
    /* Count mirrors placed by the model */
    int mirror_count = 0;
    for (i = 0; i < gH; i++)
      for (j = 0; j < gW; j++)
        if (grid[i][j] == '.' && S.model[var_M(i, j)])
    ...
    if (verify()) {
      ...
      fprintf(stderr, "c solved in %d iterations, %d mirrors\n",
              iter + 1, mirror_count);
      printf("%d %d %d\n", gW, gH, gL);
      for (i = 0; i < gH; i++) {
        for (j = 0; j < gW; j++)
    ...
    fprintf(stderr, "c iter %d: phantom cycle, adding constraint\n", iter);
    add_nogood_phantom(&S);
    free(S.DB);
  }
